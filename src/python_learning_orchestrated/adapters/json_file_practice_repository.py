...
class JsonFilePracticeRepository(PracticeRepository):
    ...
    def save_items(self, items: list[LearningItem]) -> None:
        ...
        # ⚡ Bolt: Optimize by updating dicts in-place instead of O(N) deserialization
        updates = {item.id: _item_to_dict(item) for item in items}

        for i, entry in enumerate(raw_items):
            if isinstance(entry, dict) and "id" in entry and entry["id"] in updates:
                raw_items[i] = updates.pop(entry["id"])

        for item_dict in updates.values():
