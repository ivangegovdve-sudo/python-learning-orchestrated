...
def test_cli_session_command_runs_practice_loop(capsys) -> None:
    ...
    assert "Activity: What is a variable in Python?" in output
    assert "Recorded outcome: correct." in output
...
def test_cli_checkpoint_create_fails_on_slug_collision(
    ...
    try:
        ...
        assert (
            str(exc)
            == "checkpoint 'Week-1' conflicts with existing checkpoint 'Week 1'; "
